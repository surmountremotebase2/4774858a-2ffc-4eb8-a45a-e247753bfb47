...
def handle_data(context, data):
    for symbol in context.stocks + context.cryptos + context.forex:
        ...
        rsi = data[symbol].get("rsi_14", None)
        ema9 = data[symbol].get("ema_9", None)
        ema20 = data[symbol].get("ema_20", None)
        vwap = data[symbol].get("vwap", None)

        if None in [rsi, ema9, ema20, vwap]:
            continue  # skip if any indicator is missing
